...
import_template_file = ROOT_FOLDER / "Core" / "WorkDefinitionTemplate.xlsm"

AGILE_WSDL = "http://pagapps1.ichorsystems.com:7001/CoreService/services/Table?wsdl"

AGILE_USER, AGILE_PASS = get_agile_credentials()
...
_session.auth = HTTPBasicAuth(AGILE_USER, AGILE_PASS)
...
def _normalize_fbdi_header(value):
    """Normalize an Oracle interface-column name from row 5."""
    if value is None:
        return None
    return " ".join(str(value).split())


def _find_fbdi_column(worksheet, header_name):
    """Find a column by its Oracle row-5 interface name."""
    expected = _normalize_fbdi_header(header_name)
    for column in range(1, worksheet.max_column + 1):
        actual = _normalize_fbdi_header(
            worksheet.cell(row=FBDI_HEADER_ROW, column=column).value
        )
        if actual == expected:
            return column
    raise ValueError(
        f"Missing row-5 header '{header_name}' in sheet '{worksheet.title}'"
    )


def _build_fbdi_column_map(ws_template, ws_output):
    """Map source columns to output columns using Oracle's row-5 names."""
    output_columns = {}
    for column in range(2, ws_output.max_column + 1):
        header = _normalize_fbdi_header(
            ws_output.cell(row=FBDI_HEADER_ROW, column=column).value
        )
        if header:
            if header in output_columns:
                raise ValueError(
                    f"Duplicate row-5 header '{header}' in sheet '{ws_output.title}'"
                )
            output_columns[header] = column

    column_map = {}
    for column in range(2, ws_template.max_column + 1):
        header = _normalize_fbdi_header(
            ws_template.cell(row=FBDI_HEADER_ROW, column=column).value
        )
        if header in output_columns:
            column_map[column] = output_columns[header]

    if not column_map:
        raise ValueError(
            f"No matching row-5 headers found for sheet '{ws_output.title}'"
        )
...
def write_batch_to_import_file(ws_import_batches, batch_code: str):
    ...
    batch_code_column = _find_fbdi_column(ws_import_batches, "INTERFACE_BATCH_CODE")

    batch_name_column = _find_fbdi_column(ws_import_batches, "INTERFACE_BATCH_NAME")
    ws_import_batches.cell(row=6, column=batch_code_column, value=batch_code)
    ws_import_batches.cell(row=6, column=batch_name_column, value=batch_code)
...
def write_work_definition_headers(ws_input, ws_template_headers, ws_output_headers, batch_code: str):
    ...
    column_map = _build_fbdi_column_map(ws_template_headers, ws_output_headers)
    ...
    for wd_num, tmpl in wd_list:
        ...
        for r in ws_template_headers.iter_rows(min_row=2, max_col=max_cols, values_only=True):
            ...
            for source_column, output_column in column_map.items():
                value = row_data[source_column - 2]

                ws_output_headers.cell(row=write_row, column=output_column, value=value)
...
def write_work_definition_operations(
    ...
    column_map = _build_fbdi_column_map(ws_template_ops, ws_output_ops)
    ...
    for wd_num, tmpl in wd_list:
        ...
        for r in ws_template_ops.iter_rows(min_row=2, max_col=max_cols, values_only=True):
            ...
            for source_column, output_column in column_map.items():
                ...
                ws_output_ops.cell(row=write_row, column=output_column, value=value)
...
def write_operation_resources(
    ...
    column_map = _build_fbdi_column_map(ws_template_res, ws_output_res)
    ...
    for wd_num, tmpl in wd_list:
        ...
        for r in ws_template_res.iter_rows(min_row=2, max_col=max_cols, values_only=True):
            ...
            for source_column, output_column in column_map.items():
                ...
                ws_output_res.cell(row=write_row, column=output_column, value=value)
...
def write_operation_items_standard(
    ...
    column_map = _build_fbdi_column_map(ws_template_items, ws_output_items)
    ...
    for wd_num, tmpl in wd_list:
        ...
        for (child_pn, child_qty) in children:
            ...
            for source_column, output_column in column_map.items():
                ...
                ws_output_items.cell(row=write_row, column=output_column, value=value)
